...
const name = "ModulesShortSummary"
...
func (f *FunctionCall) ToolDefinition() llms.Tool {
	return llms.Tool{
		...
		Function: &llms.FunctionDefinition{
			Name:        name,
			Description: "Returns the short summaries of a number of modules related to a query",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"query": map[string]any{
						"type":        "string",
						"description": "The query to find the modules related to",
					},
					// "unit": map[string]any{
					// 	"type": "string",
...
func Function(queryString string) string {
	searchDocs := chromaclient.PerformModuleQuery(queryString, 5)
	var dbids []int
	for _, searchDoc := range searchDocs {
		dbids = append(dbids, searchDoc.Dbid)
	}
	summaryString := cypherhelpers.PrintModuleShortSummaries(dbids)
	return "The following modules were found:\n" + summaryString
