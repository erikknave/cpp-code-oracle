...
func (f *FunctionCall) ToolDefinition() llms.Tool {
	return llms.Tool{
		...
		Function: &llms.FunctionDefinition{
			Name:        name,
			Description: "Returns a summary of the latest commits in the package (if there are any). The function will return who updated the package, when it was updated, and a summary of the changes. (Note that it might contain information on changes done also outside of this package)",
...
const queryString = `
MATCH (r:Repository)-[:HAS_MODULE]->(m:Module)-[:PART_OF_MODULE]-(p:Package {dbid:"%d"})-[:CONTAINS]-(f:File)-[:AFFECTS]-(fc:FileCommit)
