...
func (f FunctionCall) ToolDefinition() llms.Tool {
	return llms.Tool{
		...
		Function: &llms.FunctionDefinition{
			Name:        name,
			Description: "Asks a question to the repo agent (specializing and having all information regarding the repository) regarding a specific repository. The more information and context the question has, the better the answer.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"question": map[string]any{
						"type":        "string",
						"description": "The question to ask the repo agent.",
					},
					"repositorySearchId": map[string]any{
						"type":        "string",
						"description": "The search id of the repository the question is related to.",
					},
					// "unit": map[string]any{
					// 	"type": "string",
				...
				"required": []string{"question", "repositorySearchId"},
			},
		},
	}
}
...
func (f FunctionCall) Execute(args json.RawMessage) (string, error) {
	...
	webhelpers.SendServerMessageToUser(f.Context, &f.User, fmt.Sprintf("A question was sent to the Repository agent: %v..\n", helpers.SafeSubstring(params.Question, 150)))
	response := f.Function(params.Question, params.RepositorySearchId)
	webhelpers.SendServerMessageToUser(f.Context, &f.User, fmt.Sprintf("The Repository agent responded: %v..\n", helpers.SafeSubstring(response, 150)))
	return response, nil
