...
func (f *FunctionCall) ToolDefinition() llms.Tool {
	return llms.Tool{
		...
		Function: &llms.FunctionDefinition{
			Name:        name,
			Description: "Returns a summary of the latest commits in the repository (if there are any). The function will return who updated the repository, when it was updated, and a summary of the changes.",
...
const queryString = `
MATCH (r:Repository {dbid:"%d"})-[:HAS_MODULE]->(m:Module)-[:PART_OF_MODULE]-(p)-[:CONTAINS]-(f)-[:AFFECTS]-(fc:FileCommit)
